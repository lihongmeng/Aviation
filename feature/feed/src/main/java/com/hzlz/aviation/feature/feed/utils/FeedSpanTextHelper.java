...
import com.hzlz.aviation.feature.feed.view.FeedLabelTitleSpan;
import com.hzlz.aviation.kernel.base.model.video.PendantModel;
import com.hzlz.aviation.library.widget.widget.VerticalImageSpan;

/**
 * feed 富文本辅助类
 */
public class FeedSpanTextHelper {
     ...
     * @param colorInt  文字颜色
     */
    public static void createTagTitle(Context context, TextView textView, String text, @FeedTagHelper.FeedTagType int type,
                                      @ColorInt int colorInt) {
        if (context == null || textView == null) {
            return;
        }
        SpannableString spanText;
        if (FeedTagHelper.isModelValid(type)) {
            spanText = new SpannableString(PLACE_HOLDER + text);
            FeedLabelTitleSpan titlePrefixSpan = new FeedLabelTitleSpan(context, type);
    ...
    public static void createPendantTitle(Context context, TextView textView,
        ...
        int textWidth = (int) context.getResources().getDimension(
                R.dimen.feed_pendant_item_width);
        ...
        // 后缀图片
        Drawable drawable = context.getResources().getDrawable(R.drawable.feed_pendant_more);
    ...
    private static int getAvailableNum(Context context, String str, int width){
        ...
        paint.setTextSize(context.getResources().getDimension(R.dimen.feed_pendant_dimen_size));
        //根据宽度得到字符数量
