package com.jxntv.live.liveroom.live.entity;
...
public class TCUserMgr {
    ...
    public void register(final String userId, final String password, final TCHTTPMgr.Callback callback) {
        try {
            ...
            JSONObject body = new JSONObject()
                    .put("userid", userId)
                    .put("password", pwd);
            TCHTTPMgr.getInstance().request(Constants.APP_SVR_URL + "/register", body, callback);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private void loginInternal(final String userId, final String pwd, final TCHTTPMgr.Callback callback) {
        try {
            ...
            if (!TextUtils.isEmpty(Constants.APP_SVR_URL)) {
                TCHTTPMgr.getInstance().request(Constants.APP_SVR_URL + "/login", body, new TCHTTPMgr.Callback() {
                    @Override
                ...
                mUserId = userId;
                mSdkAppID = LiveManager.getInstance().getAppId();
                mNickName = LiveManager.getInstance().getNickName();
                mUserAvatar = LiveManager.getInstance().getUserAvatar();
                mUserSig = LiveManager.getInstance().getUserSig();
                // 登录到 MLVB 组件
                loginMLVB(new IMLVBLiveRoomListener.LoginCallback() {
                    @Override
                    public void onError(int errCode, String errInfo) {
                        LogUtils.e(TAG, "onError: errorCode = " + errInfo + " info = " + errInfo);
                    }
    ...
    public void fetchUserInfo(final TCHTTPMgr.Callback callback) {
        if (!TextUtils.isEmpty(Constants.APP_SVR_URL)) {
            JSONObject body = new JSONObject();
            TCHTTPMgr.getInstance().requestWithSign(Constants.APP_SVR_URL + "/get_user_info", body, new TCHTTPMgr.Callback() {
                @Override
                public void onSuccess(JSONObject data) {
                    if (data != null) {
...
//            if (AccountUtils.isLogin(mContext)){
//                AccountEntity entity = AccountUtils.getAccountEntity(mContext);
//                mUserAvatar = entity.getThumb();
    ...
    public void uploadUserInfo(final TCHTTPMgr.Callback callback) {
        if (!TextUtils.isEmpty(Constants.APP_SVR_URL)) {
            try {
                ...
                TCHTTPMgr.getInstance().requestWithSign(Constants.APP_SVR_URL + "/upload_user_info", body, callback);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
