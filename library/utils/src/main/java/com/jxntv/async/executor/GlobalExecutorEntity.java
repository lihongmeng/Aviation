package com.jxntv.async.executor;
...
public class GlobalExecutorEntity {
    ...
    protected void setThread(GlobalTask task) {
        ...
        // 2. 设置thread级别
        switch (priority) {
            case com.jxntv.async.GlobalExecutor.PRIORITY_USER:
                currentThread.setPriority(GlobalExecutorConfig.USER_THREAD_PRIORITY);
            case com.jxntv.async.GlobalExecutor.PRIORITY_NORMAL:
                currentThread.setPriority(GlobalExecutorConfig.NORMAL_THREAD_PRIORITY);
            case com.jxntv.async.GlobalExecutor.PRIORITY_BACKGROUND:
