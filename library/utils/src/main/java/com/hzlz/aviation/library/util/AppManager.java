...
public class AppManager {
    ...
    /**
     * 获取当前Activity（堆栈中ShareDataModel最后一个压入的）
     */
     ...
     * 获取指定倒数位置的Activity
     *
     * @param offsetIndex 指定倒数第几
     *
     * @return Activity
     */
    public Activity getLastWithIndexActivity(int offsetIndex) {
        if (!activityStackCheck()) {
            return null;
        }
        int length = activityStack.size();
        int index = length - offsetIndex - 1;
        if (index < 0) {
            return null;
        }
    ...
    public void finishActivity(Activity activity) {
        if (activity != null && activityStackCheck()) {
    ...
    public void removeFragment(Fragment fragment) {
        if (fragment != null && fragmentStackCheck()) {
     ...
     * @return
     */
    private boolean activityStackCheck() {
     ...
     * @return
     */
    private boolean fragmentStackCheck() {
        return fragmentStack != null && fragmentStack.size() > 0;
    }
}