package com.hzlz.aviation.library.videocache.common;

import android.content.Context;

import com.hzlz.aviation.library.videocache.HttpProxyCacheServer;
import com.hzlz.aviation.library.videocache.Logger;
...
public class PreloadManager {
    ...
    /**
     * 预加载的大小，每个视频预加载1MB，这个参数可根据实际情况调整
     */
    ...
    public void addPreloadTask(String rawUrl) {
        if (rawUrl==null || rawUrl.length()==0 || isLiveStream(rawUrl)){
            return;
        }
        if (isPreloaded(rawUrl)) return;
        PreloadTask task = new PreloadTask();
     ...
     * 是否直播流地址
     */
    private boolean isLiveStream(String rawUrl){
        return rawUrl.startsWith("rtmp://")
                || (rawUrl.startsWith("http://") || rawUrl.startsWith("https://")) && rawUrl.contains(".flv")
                || (rawUrl.startsWith("http://") || rawUrl.startsWith("https://")) && rawUrl.contains(".m3u8");
    }
    ...
    public void pausePreload(int position, boolean isReverseScroll) {
        Logger.debug("pausePreload：" + position + " isReverseScroll: " + isReverseScroll);
    ...
    public void resumePreload(int position, boolean isReverseScroll) {
        Logger.debug("resumePreload：" + position + " isReverseScroll: " + isReverseScroll);
    ...
    public String getPlayUrl(String rawUrl) {
        if (rawUrl==null || isLiveStream(rawUrl)){
            return rawUrl;
        }
        PreloadTask task = mPreloadTasks.get(rawUrl);
        if (task != null) {
        ...
        if (rawUrl.length()!=0 && isPreloaded(rawUrl)) {
            rawUrl = mHttpProxyCacheServer.getProxyUrl(rawUrl);
        }
        Logger.debug("视频播放地址："+rawUrl);
        return rawUrl;
    }
