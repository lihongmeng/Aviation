...
public class ConversationCommonHolder extends ConversationBaseHolder {
    ...
    public ConversationCommonHolder(View itemView) {
        ...
        conversationTag = rootView.findViewById(R.id.conversation_tag);
    ...
    public void layoutViews(ConversationInfo conversation, int position) {
        ...
        if (lastMsg != null && lastMsg.getStatus() == ConversationMessageInfo.MSG_STATUS_REVOKE) {
            if (lastMsg.isSelf()) {
                ...
                String nickName = conversation.getConversation().getLastMessage().getNickName();
                String message = nickName + TUIConversationConstants.covert2HTMLString(
                ...
                if (lastMsg.getMsgType() == ConversationMessageInfo.MSG_TYPE_CUSTOM) {
                    ...
                    if (lastMsg.getExtra() != null) {
                        ...
                        if (conversation.isGroup()){
                            if (!TextUtils.isEmpty(lastMsg.getNickName())){
                                if(lastMsg.isSelf()){
                                    result = "我：" + result;
                                }else {
                                    result = lastMsg.getNickName() + "：" + result;
                                }
                            }
                        }
                        if (result.startsWith("["+ PluginManager.get(H5EntryPlugin.class).getScheme()+"://")) {
                            int i = result.indexOf("]") + 1;
                            result = result.substring(Math.max(i, 0));
                        }
        ...
        if (PluginManager.get(ChatIMPlugin.class).isPlatformAccount(conversation.getId())){
            conversationTag.setVisibility(View.VISIBLE);
        }else {
            conversationTag.setVisibility(View.GONE);
        }
