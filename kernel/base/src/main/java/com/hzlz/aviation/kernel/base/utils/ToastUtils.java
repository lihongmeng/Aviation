...
public final class ToastUtils {
    ...
    private static int yOffset         = (int) (64 * GVideoRuntime.getAppContext().getResources().getDisplayMetrics().density + 0.5);
    ...
    public static void setView(@LayoutRes int layoutId) {
        LayoutInflater inflate = (LayoutInflater) GVideoRuntime.getAppContext().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
     ...
     * @param resId 资源Id
     */
    public static void showShortCenter(@StringRes int resId) {
    ...
    private static void show(@StringRes int resId, int duration) {
        show(GVideoRuntime.getAppContext().getResources().getText(resId).toString(), duration);
    }
    ...
    private static void show(@StringRes int resId, int duration, Object... args) {
        show(String.format(GVideoRuntime.getAppContext().getResources().getString(resId), args), duration);
    }
    ...
    private static void show(CharSequence text, int duration) {
        ...
        if (sViewWeakReference != null) {
            final View view = sViewWeakReference.get();
            if (view != null) {
                sToast = new Toast(GVideoRuntime.getAppContext());
                sToast.setView(view);
        ...
        if (!isCustom) {
            if (messageColor != DEFAULT_COLOR) {
                ...
                sToast = Toast.makeText(GVideoRuntime.getAppContext(), spannableString, duration);
            } else {
                sToast = Toast.makeText(GVideoRuntime.getAppContext(), text, duration);
            }
        }
